...
class TestAccountSystem:
    ...
    def test_debit_insufficient_funds(self):
        # Test that debiting more than the available balance raises an error
        # This simulates an overdraft attempt, which should not be allowed
        self.acc.balance = 1000.00
        with pytest.raises(AssertionError):
    ...
    def test_debit_negative_amount(self):
        ...
        # Negative debits are invalid and should be rejected
        self.acc.balance = 1000.00
        with pytest.raises(AssertionError):
            assert -50.00 > 0

    def test_view_balance(self, capsys):
        # Test that viewing the balance prints the correct value
        # Uses capsys to capture printed output for assertion
    ...
    def test_exit_message(self, capsys):
        # Test that the exit message is printed correctly
        # Simulates program exit and checks output
